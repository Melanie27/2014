...
function display_movie_review_meta_box( $movie_review ) {
    ...
    $movie_caption = esc_html( get_post_meta( $movie_review->ID, 'movie_caption', true ) );
    $movie_date = esc_html( get_post_meta( $movie_review->DATE, 'movie_date', true ) );
    ?>
    <table>
        <tr>
        ...
        <tr>
            <td style="width: 100%">Caption</td>
            <td><input type="text" size="80" name="movie_review_caption" value="<?php echo $movie_caption; ?>" /></td>
        </tr>
         <tr>
            <td style="width: 100%">Date</td>
            <td><input type="text" size="80" name="movie_review_date" value="<?php echo $movie_date; ?>" /></td>
        </tr>
        <tr>
...
function add_movie_review_fields( $movie_review_id, $movie_review ) {
    ...
    if ( $movie_review->post_type == 'movie_reviews' ) {
        ...
        if ( isset( $_POST['movie_review_rating'] ) && $_POST['movie_review_rating'] != '' ) {
         ...
         if ( isset( $_POST['movie_review_caption'] ) && $_POST['movie_review_caption'] != '' ) {
            update_post_meta( $movie_review_id, 'movie_caption', $_POST['movie_review_caption'] );
        }
    }
}
?>
